...
if(mod[1]=='l')
{
	l_m_a_m <- matrix(NA, num_m, 2)
	l_m_a_m[,1] <- l_m_1
	l_m_a_m[,2] <- l_m_2
	l_m_a_d <- matrix(NA, num_d, 2)
	l_m_a_d[,1] <- l_d_1
	l_m_a_d[,2] <- l_d_2
	D_a_t <- matrix(0,2,2)
}
if(mod[1]=='c')
{
	l_m_a_m <- matrix(1, num_m, 1)
	l_m_a_d <- matrix(1, num_d, 1)
	D_a_t <- matrix(0,1,1)
}
if(mod[2]=='l')
{
	l_m_c_m <- matrix(NA, num_m, 2)
	l_m_c_m[,1] <- l_m_1
	l_m_c_m[,2] <- l_m_2
	l_m_c_d <- matrix(NA, num_d, 2)
	l_m_c_d[,1] <- l_d_1
	l_m_c_d[,2] <- l_d_2
	D_c_t <- matrix(0,2,2)
}
if(mod[2]=='c')
{
	l_m_c_m <- matrix(1, num_m, 1)
	l_m_c_d <- matrix(1, num_d, 1)
	D_c_t <- matrix(0,1,1)
}
if(mod[3]=='l')
{
	l_m_e_m <- matrix(NA, num_m, 2)
	l_m_e_m[,1] <- l_m_1
	l_m_e_m[,2] <- l_m_2
	l_m_e_d <- matrix(NA, num_d, 2)
	l_m_e_d[,1] <- l_d_1
	l_m_e_d[,2] <- l_d_2
	D_e_t <- matrix(0,2,2)
}
if(mod[3]=='c')
{
	l_m_e_m <- matrix(1, num_m, 1)
	l_m_e_d <- matrix(1, num_d, 1)
	D_e_t <- matrix(0,1,1)
}
...
{
	#result <- mcmc_epsp_AtCtEt(pheno_m, pheno_d, B_des_a_m, B_des_a_d, B_des_c_m, B_des_c_d, B_des_e_m, B_des_e_d, -1, -1, -1, D_a, D_c, D_e, 10000, 500, 0.1)
	#beta_a <- result$beta_a_mc
	#beta_c <- result$beta_c_mc
	#beta_e <- result$beta_e_mc
	#nll <- .Call('loglik_AtCtEt_epsp_g_c', beta_a, beta_c, beta_e, pheno_m, pheno_d, B_des_a_m, B_des_a_d, B_des_c_m, B_des_c_d, B_des_e_m, B_des_e_d, lower, lower, lower, D_a, D_c, D_e)
	low_a <- -8
	upp_a <- 10
	low_c <- -8
	upp_c <- 10
	low_e <- -5
	upp_e <- 10
	result <- optim(c(beta_a,beta_c,beta_e), loglik_AtCtEt_epsp_g, gr_AtCtEt_epsp_g, pheno_m = pheno_m, pheno_d = pheno_d, B_des_a_m=l_m_a_m, B_des_a_d=l_m_a_d, B_des_c_m=l_m_c_m, B_des_c_d=l_m_c_d, B_des_e_m=l_m_e_m, B_des_e_d=l_m_e_d, var_b_a=0, var_b_c=0, var_b_e=0, D_a=D_a_t, D_c=D_c_t, D_e=D_e_t, lower = c(rep(low_a,n_a),rep(low_c,n_c),rep(low_e,n_e)), upper = c(rep(upp_a,n_a),rep(upp_c,n_c),rep(upp_e,n_e)), method = "L-BFGS-B", control=list(maxit = 3000))
	beta_a <- result$par[1:n_a]
	beta_c <- result$par[(1+n_a):(n_a+n_c)]
	beta_e <- result$par[(1+n_a+n_c):(n_a+n_c+n_e)]		
	AtCtEtp_model <- list(D_a = D_a, D_c = D_c, D_e = D_e, pheno_m = pheno_m, pheno_d = pheno_d, T_m = T_m, T_d = T_d, knot_a=knots_a, knot_c=knots_c, knot_e=knots_e, beta_a=beta_a, beta_c=beta_c, beta_e=beta_e, con=result$convergence, lik=result$value/2, iter=result$value/2, var_b_a=lower, var_b_c=lower, var_b_e=lower)
	...
	while(abs(lik-lik_pre)>eps)
	{
		...
		{	
			...
			if(mod[3]!='d')
			{v_e_t <- 0}
			#result <- mcmc_epsp_AtCtEt(pheno_m, pheno_d, B_des_a_m, B_des_a_d, B_des_c_m, B_des_c_d, B_des_e_m, B_des_e_d, v_a_t, v_c_t, v_e_t, D_a, D_c, D_e, 3000, 500, 0.1)
			#beta_a <- result$beta_a_mc
			#beta_c <- result$beta_c_mc
			#beta_e <- result$beta_e_mc
			...
			result <- optim(c(beta_a,beta_c,beta_e), loglik_AtCtEt_epsp_g, gr_AtCtEt_epsp_g, pheno_m = pheno_m, pheno_d = pheno_d, B_des_a_m=l_m_a_m, B_des_a_d=l_m_a_d, B_des_c_m=l_m_c_m, B_des_c_d=l_m_c_d, B_des_e_m=l_m_e_m, B_des_e_d=l_m_e_d, var_b_a=v_a_t, var_b_c=v_c_t, var_b_e=v_e_t, D_a=D_a_t, D_c=D_c_t, D_e=D_e_t, lower = c(rep(low_a,n_a),rep(low_c,n_c),rep(low_e,n_e)), upper = c(rep(upp_a,n_a),rep(upp_c,n_c),rep(upp_e,n_e)), method = "L-BFGS-B", control=list(maxit = 3000))
			beta_a <- result$par[1:n_a]
			beta_c <- result$par[(1+n_a):(n_a+n_c)]
			beta_e <- result$par[(1+n_a+n_c):(n_a+n_c+n_e)]
			...
			#if(mod[1]=='l')
			#{
			#	 i_s_a <- solve(splineDesign(knots_a, x=c(min(T_m,T_d),max(T_m,T_d)), ord=order)%*%cbind(rep(1,n_a), 0:(n_a-1)))%*%c(beta_a[1],beta_a[2])
			#	 beta_a <- rep(1,n_a)*i_s_a[1] + (0:(n_a-1))*i_s_a[2]
			#}
			#if(mod[1]=='c')
			#{
			#	beta_a <- rep(1,n_a)*beta_a[1]
			#}
			#if(mod[2]=='l')
			#{
			#	i_s_c <- solve(splineDesign(knots_c, x=c(min(T_m,T_d),max(T_m,T_d)), ord=order)%*%cbind(rep(1,n_c), 0:(n_c-1)))%*%c(beta_c[1],beta_c[2])
			#	beta_c <- rep(1,n_c)*i_s_c[1] + (0:(n_c-1))*i_s_c[2]
			#}
			#if(mod[2]=='c')
			#{
			#	beta_c <- rep(1,n_c)*beta_c[1]
			#}
			#if(mod[3]=='l')
			#{
			#	i_s_e <- solve(splineDesign(knots_e, x=c(min(T_m,T_d),max(T_m,T_d)), ord=order)%*%cbind(rep(1,n_e), 0:(n_e-1)))%*%c(beta_e[1],beta_e[2])
			#	beta_e <- rep(1,n_e)*i_s_e[1] + (0:(n_e-1))*i_s_e[2]
			#}
			#if(mod[3]=='c')
			#{
			#	beta_e <- rep(1,n_e)*beta_e[1]
			#}
			betas <- rbind(betas,c(beta_a,beta_c,beta_e))
			...
			while(result$con!=0)
			{
				var_b_a_t <- ifelse(var_b_a==0, var_b_a, runif(1, min=0.5, max=2))
				var_b_c_t <- ifelse(var_b_c==0, var_b_c, runif(1, min=0.5, max=2))
				var_b_e_t <- ifelse(var_b_e==0, var_b_e, runif(1, min=0.5, max=2))
	...
	if((mod[1]=='d')&(mod[2]=='d')&(mod[3]=='d'))
	{
		...
		# AtCtEtp_model <- list(D_a = D_a, D_c = D_c, D_e=D_e, pheno_m = pheno_m, pheno_d = pheno_d, T_m = T_m, T_d = T_d, knot_a=knots_a, knot_c=knots_c, knot_e=knots_e, beta_a=result$par[1:n_a], beta_c=result$par[(1+n_a):(n_c+n_a)], beta_e=result$par[(1+n_a+n_c):(n_c+n_a+n_e)], con=result$convergence, lik=(result$value)/2, iter=(liks)/2, var_b_a=vars[min_i,1], var_b_c=vars[min_i,2], var_b_e=vars[min_i,3])	
		AtCtEtp_model <- list(D_a = D_a, D_c = D_c, D_e=D_e, pheno_m = pheno_m, pheno_d = pheno_d, T_m = T_m, T_d = T_d, knot_a=knots_a, knot_c=knots_c, knot_e=knots_e, beta_a=result$par[1:n_a], beta_c=result$par[(1+n_a):(n_c+n_a)], beta_e=result$par[(1+n_a+n_c):(n_c+n_a+n_e)], con=result$convergence, lik=min(liks)/2, iter=(liks)/2, var_b_a=vars[min_i,1], var_b_c=vars[min_i,2], var_b_e=vars[min_i,3])	
	}else{
		v_a_t <- var_b_a
		v_c_t <- var_b_c
		v_e_t <- var_b_e
		...
		#AtCtEtp_model <- list(D_a = D_a, D_c = D_c, D_e=D_e, pheno_m = pheno_m, pheno_d = pheno_d, T_m = T_m, T_d = T_d, knot_a=knots_a, knot_c=knots_c, knot_e=knots_e, beta_a=result$beta_a_mc, beta_c=result$beta_c_mc, beta_e=result$beta_e_mc, con=0, lik=nll, iter=(liks)/2, var_b_a=vars[min_i,1], var_b_c=vars[min_i,2], var_b_e=vars[min_i,3])	
		...
		#	beta_e <- rep(1,n_e)*beta_e[1]
		#}
		# AtCtEtp_model <- list(D_a = D_a, D_c = D_c, D_e=D_e, pheno_m = pheno_m, pheno_d = pheno_d, T_m = T_m, T_d = T_d, knot_a=knots_a, knot_c=knots_c, knot_e=knots_e, beta_a=beta_a, beta_c=beta_c, beta_e=beta_e, con=result$convergence, lik=(result$value)/2, iter=(liks)/2, var_b_a=vars[min_i,1], var_b_c=vars[min_i,2], var_b_e=vars[min_i,3])	
		AtCtEtp_model <- list(D_a = D_a, D_c = D_c, D_e=D_e, pheno_m = pheno_m, pheno_d = pheno_d, T_m = T_m, T_d = T_d, knot_a=knots_a, knot_c=knots_c, knot_e=knots_e, beta_a=beta_a, beta_c=beta_c, beta_e=beta_e, con=result$convergence, lik=min(liks)/2, iter=(liks)/2, var_b_a=vars[min_i,1], var_b_c=vars[min_i,2], var_b_e=vars[min_i,3])	
