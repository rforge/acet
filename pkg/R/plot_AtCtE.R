plot_AtCtE <- function(AtCtE, data_m, data_d)
{
	...
	plot(range(x), c(0,max(points_c, points_a)+1), type = "n", xlab = "Age", ylab = "Variance",main =  "Variance curves of the A, C, and E components")
	...
	points_e <- rep(model_cur$var, length(x))
	...
	legend(x[1], max(points_c, points_a, model_cur$var)+1, c('Additive genetic component','Common environmental component', 'Unique environmental component'), col = c('red','blue','pink'), lty=c(1,1,1), lwd=c(2,2,2))
