...
var_b_a <- runif(1,min=0.5,max=1.5)
if(mod[1] %in% c('l','c'))
{
	var_b_a <- lower
	n_a <- ifelse(mod[1]=='l',2,1) 
}
var_b_c <- runif(1,min=0.5,max=1.5)
if(mod[2] %in% c('l','c'))
{
	var_b_c <- lower
	n_c <- ifelse(mod[2]=='l',2,1) 
}
var_b_e <- runif(1,min=0.5,max=1.5)
if(mod[3] %in% c('l','c'))
{
	var_b_e <- lower
...
if((mod[1]!='d')&(mod[2]!='d')&(mod[3]!='d'))
{
	...
	while(abs(lik-lik_pre)>eps)
	{
		...
		if((mod[1]=='d')&(mod[2]=='d')&(mod[3]=='d'))
		{
			result <- optim(c(beta_a,beta_c,beta_e), loglik_AtCtEt_epsp_g, gr_AtCtEt_epsp_g, pheno_m = pheno_m, pheno_d = pheno_d, B_des_a_m=B_des_a_m, B_des_a_d=B_des_a_d, B_des_c_m=B_des_c_m, B_des_c_d=B_des_c_d, B_des_e_m=B_des_e_m, B_des_e_d=B_des_e_d, var_b_a=var_b_a, var_b_c=var_b_c, var_b_e=var_b_e, D_a=D_a, D_c=D_c, D_e=D_e, lower = c(rep(-10,n_a+n_c),rep(-9,n_e)), upper = c(rep(10,n_a+n_c),rep(9,n_e)), method = "L-BFGS-B", control=list(maxit = 3000))
			betas <- rbind(betas, result$par)
			beta_a <- result$par[1:n_a]
			beta_c <- result$par[(1+n_a):(n_a+n_c)]
			beta_e <- result$par[(1+n_a+n_c):(n_a+n_c+n_e)]
      
			result <- optim(c(var_b_a,var_b_c,var_b_e), loglik_AtCtEt_epsp, gr_AtCtEt_epsp, pheno_m = pheno_m, pheno_d = pheno_d, B_des_a_m=B_des_a_m, B_des_a_d=B_des_a_d, B_des_c_m=B_des_c_m, B_des_c_d=B_des_c_d, B_des_e_m=B_des_e_m, B_des_e_d=B_des_e_d, beta_a=beta_a, beta_c=beta_c, beta_e = beta_e, D_a=D_a, D_c=D_c, D_e=D_e, lower = rep(1e-6,3), upper = rep(20,3), method = "L-BFGS-B", control=list(maxit = 3000))
		}else
		{	
			v_a_t <- var_b_a
			v_c_t <- var_b_c
	...
	if((mod[1]=='d')&(mod[2]=='d')&(mod[3]=='d'))
	{
		result <- optim(betas[min_i,], loglik_AtCtEt_epsp_g, gr_AtCtEt_epsp_g, pheno_m = matrix(pheno_m), pheno_d = matrix(pheno_d), B_des_a_m=B_des_a_m, B_des_a_d=B_des_a_d, B_des_c_m=B_des_c_m, B_des_c_d=B_des_c_d, B_des_e_m=B_des_e_m, B_des_e_d=B_des_e_d, var_b_a=vars[min_i,1], var_b_c=vars[min_i,2], var_b_e=vars[min_i,3], D_a=D_a, D_c=D_c, D_e=D_e, lower = c(rep(-15,n_a+n_c),rep(-10,n_e)), upper = c(rep(15,n_a+n_c),rep(10,n_e)), method = "L-BFGS-B", control=list(maxit = 3000))
	}else{
		v_a_t <- vars[min_i,1]
		v_c_t <- vars[min_i,2]
		v_e_t <- vars[min_i,3]
	...
	beta_a <- result$par[1:n_a]
		beta_c <- result$par[(1+n_a):(n_a+n_c)]
		beta_e <- result$par[(1+n_a+n_c):(n_a+n_c+n_e)]
    #bf <- loglik_AtCtEt_epsp(vars[min_i,], pheno_m=pheno_m, pheno_d=pheno_d, B_des_a_m=B_des_a_m, B_des_a_d=B_des_a_d, beta_a=beta_a, D_a=D_a, B_des_c_m=B_des_c_m, B_des_c_d=B_des_c_d, beta_c=beta_c, D_c=D_c, B_des_e_m=B_des_e_m, B_des_e_d=B_des_e_d, beta_e=beta_e, D_e=D_e)
		
		#lik_p <- result$value/2
		#lik_sm <- loglik_AtCtEt_epsp_g(result$par, pheno_m = pheno_m, pheno_d = pheno_d, B_des_a_m=B_des_a_m, B_des_a_d=B_des_a_d, B_des_c_m=B_des_c_m, B_des_c_d=B_des_c_d, B_des_e_m=B_des_e_m, B_des_e_d=B_des_e_d, var_b_a=0, var_b_c=0, var_b_e=0, D_a=D_a, D_c=D_c, D_e=D_e)
		#lik_sm <- lik_sm/2
