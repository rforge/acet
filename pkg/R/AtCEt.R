...
result <- optim(c(1,rep(0, n_a),rep(0, n_e)), loglik_AtCEt_esp, gr_AtCEt_esp, pheno_m = matrix(pheno_m), pheno_d = matrix(pheno_d), B_des_a_m = B_des_a_m, B_des_a_d = B_des_a_d, B_des_e_m = B_des_e_m, B_des_e_d = B_des_e_d, lower = c(rep(0.00001,1),rep(-5, n_a),rep(-5, n_e)), upper = c(rep(100,1),rep(5, n_a),rep(5, n_e)), method = "L-BFGS-B", hessian = TRUE)

AtCEt_model <- list(n_beta_a=n_a, n_beta_e=n_e, var=result$par[1], beta_a=result$par[2:(1+n_a)], beta_e=result$par[(2+n_a):(1+n_a+n_e)], hessian=result$hessian, con=result$convergence, lik=result$value, knots_a =knots_a, knots_e = knots_e )

class(AtCEt_model) <- 'AtCEt_model'
